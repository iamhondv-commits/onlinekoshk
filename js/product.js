/* ==========================================================================
   أونلاين كشك - Main Interactive Script & Product Router Engine Integration
   ========================================================================== */

// 🔒 دالة تشفير وتفكيك الروابط إن أردت
window.KoshkEncoder = {
    encode: function(str) {
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // 5. تفعيل نموذج التواصل المباشر (محظر على الزوار غير المسجلين)
    initContactForm();
    ...
    if (client) {
        ...
        if (productId) {
            await loadProductDetailsEngine(productId, client);
        }
    }
});
...
// دالة تطهير وتنظيف حقل البحث من الأكواد والرموز الضارة (Input Sanitization)
...
async function loadProductDetailsEngine(id, client) {
    ...
    try {
        ...
        if (error || !prod) throw new Error('تعذر جلب تفاصيل المنتج');
        ...
        // 3. ربط زر الشراء
        setupBuyButtonEngine(prod, client);
...
function updateProductUIEngine(prod, parentCategory) {
    ...
    setElText('productPageTitle', prod.title);
    setElText('productTitle', prod.title);
    setElText('productStoreBadge', prod.store_name || 'متجر معتمد');
    setElText('productCategoryBadge', prod.category || 'عام');
    setElText('productDiscountPrice', `${prod.discount_price || 0} ج.م`);
    setElText('productOriginalPrice', `${prod.original_price || 0} ج.م`);
    ...
    if (catBread) {
        catBread.innerText = prod.category || 'عام';
        catBread.href = `categories.html?cat=${encodeURIComponent(prod.category || '')}`;
    }

    const storeBread = document.getElementById('productStoreBreadcrumb');
    if (storeBread) {
        storeBread.innerText = prod.store_name || 'المتجر';
        storeBread.href = `stores.html?name=${encodeURIComponent(prod.store_name || '')}`;
    }
    ...
    if (brandBadge) {
        if (prod.brand) {
            brandBadge.innerText = prod.brand;
    ...
    if (descEl) descEl.innerText = prod.description || 'لا يوجد وصف مضاف لهذا المنتج.';

    // المواصفات الفنية
    const specsContainer = document.getElementById('productSpecsContainer');
    const specsTable = document.getElementById('productSpecsTable')?.querySelector('tbody');

    if (specsContainer && specsTable) {
        if (prod.specs && prod.specs.trim() !== '') {
            const lines = prod.specs.split('\n').filter(l => l.trim() !== '');
            specsTable.innerHTML = lines.map(line => {
                const parts = line.split(':');
                return `
                    <tr>
                        <td class="fw-bold text-dark w-40">${(parts[0] || '').trim()}</td>
                        <td class="text-secondary">${(parts.slice(1).join(':') || parts[0]).trim()}</td>
                    </tr>
                `;
            }).join('');
...
function setupProductGalleryEngine(prod) {
    let allImgs = [];
    if (prod.image_url) allImgs.push(prod.image_url);
    ...
    if (allImgs.length === 0) {
        allImgs = ['https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600'];
    }
    ...
    const thumbsContainer = document.getElementById('thumbnailsWrapper');

    if (mainImg) mainImg.src = currentProductImages[0];

    if (thumbsContainer) {
        thumbsContainer.innerHTML = currentProductImages.map((imgUrl, index) => `
            <img src="${imgUrl}" class="product-thumb-img ${index === 0 ? 'active' : ''}" onclick="switchMainProductGalleryImg(${index})" alt="صورة ${index + 1}">
        `).join('');
    }
}

window.switchMainProductGalleryImg = function(index) {
...
function setupBuyButtonEngine(prod, client) {
    ...
    buyBtn.onclick = async function () {
        ...
        if (!user) {
            const authModalEl = document.getElementById('affiliateAuthRequiredModal');
            if (authModalEl) {
        ...
        if (prod.affiliate_url) {
            window.open(prod.affiliate_url, '_blank');
        } else {
