...
function showPage(pageName) {
    ...
    if (pageName === 'home' || pageName === 'feed') {
        ...
        initSlider();
        // إعادة تهيئة حقن مستمع الداتا بيز الفريد للكروت لمنع تشوه التصميم عند العودة للرئيسية
        if(window.syncSlidersLive) window.syncSlidersLive();
        return;
    }
    ...
    window.showPageCustomLive = function(targetPage) {
        if (targetPage === 'home' || targetPage === 'feed') {
            ...
            const wrap2 = document.getElementById('koshkHomepagePremiumFeaturesCardsArea');
