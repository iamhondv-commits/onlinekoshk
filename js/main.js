...
// 🔒 دالة تشفير وتفكيك اللينكات بالـ URL
window.KoshkEncoder = {
    encode: function(str) {
        try {
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (client) {
        ...
        // 8. تحميل البيانات المحفوظة بواسطة محرر الموقع الحي وتطبيقها للمستخدمين
...
// دالة جلب الأقسام ديناميكياً وتغذية الناف بار بجميع الصفحات بأسلوب الروابط المقروءة وبدون حذف الأقسام القديمة
...
// جلب الاقتراحات المباشرة من الداتا بيز بـ الروابط المقروءة المحولة لصفحة تفاصيل المنتج
async function fetchSearchSuggestions(query, box) {
    try {
        ...
        if (products && products.length > 0) {
            ...
            products.forEach(p => {
                const slug = createReadableSlug(p.title, p.id);
                html += `
                    <a href="product.html?p=${slug}" class="dropdown-item rounded-3 py-2 fw-bold d-flex align-items-center justify-content-between text-decoration-none">
...
// 🌟 البانرات السلايدر (المكان المخصص والمسموح بوجود رابط الأفلييت المباشر به - تم إصلاح الاستعلام أمنياً)
...
// 🌟 جلب المنتجات للرئيسية مع توجيه كافة الكروت لصفحة تفاصيل المنتج الداخلي برابط مقروء وليس الأفلييت
    ...
    try {
        ...
        if (productsContainer) {
            productsContainer.innerHTML = products.map(prod => {
                const slug = createReadableSlug(prod.title, prod.id);
                const productDetailUrl = `product.html?p=${slug}`;
